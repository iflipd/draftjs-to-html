...
function getBlockMarkup(block, entityMap, hashtagConfig, directional, customEntityTransform) {
  ...
  if (block.data && block.data.type === 'image') {
		...
		blockHtml.push('<div style="display:flex;justify-content:center;flex-direction:column;width: 100%;height: auto;align-items:center;">')
		blockHtml.push('<img src="' + block.data.src + '" alt="' + block.data.caption + '" style="height: auto;width: ' + width + 'px;"/>');
		if (block.data.caption) {
			blockHtml.push('<div style="width:'+width+'px;font-size:1em;text-align:right;">'+block.data.caption+'</div>')
		}
		if (block.data.rightsHolder) {
			blockHtml.push('<div style="width:'+width+'px;font-size:0.8em;color:#555;text-align:right;">'+block.data.rightsHolder+'</div>')
		}
		blockHtml.push('</div>')
